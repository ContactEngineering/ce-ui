...
<template>
  <div v-if="props.stage == 0">
    <div v-if="surface && !error" class="alert mt-5" style="background-color: oklch(0.852 0.199 91.936 / 0.4);">
      <h4>
    ...
    <div v-if="error" class="alert mt-5" style="background-color: oklch(0.577 0.245 27.325 / 0.4);">
      <h4>
        Sorry we could not find a digital surface twin with the ID: {{ surfaceId }}
      </h4>
    </div>


    <div v-if="surface && !error" class="d-flex flex-row justify-content-end">
      <BButton @click="$emit('continue')" variant="primary" size="lg">
        Continue
      </BButton>
    </div>
  </div>
